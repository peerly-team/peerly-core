...
internal sealed class UpdateRubricValidator : ICommandValidator<UpdateRubricCommand, Success>
{
    private readonly ICommonUnitOfWorkFactory _unitOfWorkFactory;

    public UpdateRubricValidator(ICommonUnitOfWorkFactory unitOfWorkFactory)
    {
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (await unitOfWork.ReadOnlyHomeworkRepository.ExistsPublishedByRubricIdAsync(command.RubricId, cancellationToken))
        {
            return ValidationError.From(RubricErrors.RubricReferencedByPublishedHomework);
        }

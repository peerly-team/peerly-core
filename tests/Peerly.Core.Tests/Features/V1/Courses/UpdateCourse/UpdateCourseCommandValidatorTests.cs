...
public sealed class UpdateCourseCommandValidatorTests
{
    ...
    public UpdateCourseCommandValidatorTests()
    {
        var unitOfWorkFactory = SetupUnitOfWorkFactory();
        _validator = new UpdateCourseCommandValidator(unitOfWorkFactory);
    }

    [Theory]
    [InlineData(CourseStatus.Draft)]
    [InlineData(CourseStatus.InProgress)]
    public async Task ValidateAsync_CourseTeacherExistsAndCourseCanBeUpdated_ShouldSuccess(CourseStatus courseStatus)
    {
        // Arrange
    ...
    public async Task ValidateAsync_CourseCanNotBeUpdated_ShouldBeValidationError(CourseStatus courseStatus)
    {
        // Arrange
        var command = _fixture.Create<UpdateCourseCommand>();
        ...
        result.AsT1.Errors.Should().NotBeNull().And.ContainSingle(CourseErrors.IncorrectCourseStatusForUpdate);
    }

...
public sealed class PublishCourseHandlerTests
{
    ...
    private readonly Mock<ICommandValidator<PublishCourseCommand, Success>> _validatorMock = new();

    private readonly Fixture _fixture = new();
    private readonly PublishCourseHandler _handler;

    public PublishCourseHandlerTests()
    {
        var unitOfWorkFactoryMock = SetupUnitOfWorkFactory();

        _handler = new PublishCourseHandler(
            unitOfWorkFactoryMock,
            _validatorMock.Object);
    }

    [Fact]
    public async Task ExecuteAsync_ValidationResultSuccess_ShouldPublishCourse()
    {
        // Arrange
    ...
    public async Task ExecuteAsync_ValidationResultValidationError_ShouldBeValidationError()
    {
        ...
        _validatorMock
            .Setup(validator => validator.ValidateAsync(command, It.IsAny<CancellationToken>()))
            .ReturnsAsync(ValidationError.From(CourseErrors.IncorrectCourseStatusForPublish));
        ...
        commandResponse.AsT1.Errors.Should().NotBeNull().And.ContainSingle(CourseErrors.IncorrectCourseStatusForPublish.Value);
        _unitOfWorkMock.Verify(
            unitOfWork => unitOfWork.CourseRepository.UpdateAsync(
                It.IsAny<CourseId>(),
                It.IsAny<Action<IUpdateBuilder<CourseUpdateItem>>>(),

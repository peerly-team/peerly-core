...
namespace Peerly.Core.Tests.Features.V1.Courses.PublishCourse;

public sealed class PublishCourseCommandValidatorTests
{
    private readonly Mock<ICommonUnitOfWork> _unitOfWorkMock = new();

    private readonly Fixture _fixture = new();
    private readonly PublishCourseCommandValidator _validator;

    public PublishCourseCommandValidatorTests()
    {
        var unitOfWorkFactory = SetupUnitOfWorkFactory();
        _validator = new PublishCourseCommandValidator(unitOfWorkFactory);
    }
    ...
    {
        // Arrange
        var command = _fixture.Create<PublishCourseCommand>();
    ...
    public async Task ValidateAsync_CourseNotInDraftStatus_ShouldBeValidationError(CourseStatus courseStatus)
    {
        // Arrange
        var command = _fixture.Create<PublishCourseCommand>();
        ...
        var course = _fixture.Build<Course>()
            .With(result => result.Id, command.CourseId)
            .With(result => result.Status, courseStatus)
            .Create();
        _unitOfWorkMock
            .Setup(unitOfWork => unitOfWork.CourseRepository.GetAsync(command.CourseId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(course);
        ...
        result.AsT1.Errors.Should().NotBeNull().And.ContainSingle(CourseErrors.IncorrectCourseStatusForPublish);
    }
